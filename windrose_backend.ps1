...
# ── Auto-detect paths ─────────────────────────────────────
$SCRIPT_DIR = if ($PSScriptRoot) { $PSScriptRoot } elseif ($MyInvocation.MyCommand.Path) { Split-Path $MyInvocation.MyCommand.Path } else { Split-Path ([System.Diagnostics.Process]::GetCurrentProcess().MainModule.FileName) }
$CONFIG_FILE = "$SCRIPT_DIR\windrose_gui_config.txt"

function Find-ServerDir {
    $candidates = @()
    # Registry - get Steam install path
    foreach ($reg in @("HKCU:\Software\Valve\Steam","HKLM:\Software\Valve\Steam","HKLM:\Software\Wow6432Node\Valve\Steam")) {
        try {
            $sp = (Get-ItemProperty -Path $reg -ErrorAction SilentlyContinue).SteamPath
            if ($sp) {
                $sp = $sp.Replace('/',[char]92).TrimEnd([char]92)
                $candidates += "$sp\steamapps\common\Windrose Dedicated Server"
                $vdf = "$sp\steamapps\libraryfolders.vdf"
                if (Test-Path $vdf) {
                    foreach ($line in [System.IO.File]::ReadAllLines($vdf)) {
                        if ($line -match '"path"\s+"([^"]+)"') {
                            $lib = $Matches[1].Replace('\\',[char]92)
    ...
    # Scan all drives
    $drives = (Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue | Where-Object { $_.Used -gt 0 }).Root
    foreach ($drive in $drives) {
        $d = $drive.TrimEnd([char]92)
        foreach ($f in @("Steam","SteamLibrary","SteamCMD","Games\Steam","Games\SteamLibrary","Program Files (x86)\Steam","Program Files\Steam")) {
            $candidates += "$d\$f\steamapps\common\Windrose Dedicated Server"
        }
    }
    foreach ($c in ($candidates | Select-Object -Unique)) {
        $exe = "$c\R5\Binaries\Win64\WindroseServer-Win64-Shipping.exe"
        if (Test-Path $exe -ErrorAction SilentlyContinue) { return $c }
    }
    # Check user profile folders explicitly
    $userDirs = @($env:USERPROFILE, $env:PUBLIC, "$env:USERPROFILE\Desktop", "$env:USERPROFILE\Downloads", "$env:USERPROFILE\Documents")
    $total = $userDirs.Count; $i = 0
    foreach ($dir in $userDirs) {
        $i++
        Write-Progress -Activity "Scanning user folders" -Status "$dir" -PercentComplete ([int]($i/$total*100))
        if ($dir) {
            $exe = "$dir\Windrose Dedicated Server\R5\Binaries\Win64\WindroseServer-Win64-Shipping.exe"
            if (Test-Path $exe -ErrorAction SilentlyContinue) { Write-Progress -Activity "Scanning user folders" -Completed; return "$dir\Windrose Dedicated Server" }
            Get-ChildItem -Path $dir -Directory -ErrorAction SilentlyContinue | ForEach-Object {
                $exe2 = "$($_.FullName)\R5\Binaries\Win64\WindroseServer-Win64-Shipping.exe"
                if (Test-Path $exe2 -ErrorAction SilentlyContinue) { Write-Progress -Activity "Scanning user folders" -Completed; return $_.FullName }
            }
        }
    }
    Write-Progress -Activity "Scanning user folders" -Completed
    # Last resort - full recursive search with progress
    Write-Host "  Doing full drive scan (this may take a moment)..."
    $drives = (Get-PSDrive -PSProvider FileSystem -ErrorAction SilentlyContinue | Where-Object { $_.Used -gt 0 }).Root
    $di = 0; $dtotal = @($drives).Count
    foreach ($drive in $drives) {
        $di++
        Write-Progress -Activity "Full drive scan" -Status "Scanning $drive" -PercentComplete ([int]($di/$dtotal*100))
        try {
            $found = Get-ChildItem -Path $drive -Recurse -Filter "WindroseServer-Win64-Shipping.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
            if ($found) { Write-Progress -Activity "Full drive scan" -Completed; return $found.Directory.Parent.Parent.Parent.Parent.FullName }
        } catch {}
    }
    Write-Progress -Activity "Full drive scan" -Completed
    return $null
}
...
function Find-SteamCmd {
    ...
    foreach ($drive in $drives) {
        $d = $drive.TrimEnd([char]92)
        foreach ($p in @("$d\SteamCMD\steamcmd.exe","$d\steamcmd\steamcmd.exe","$d\steamcmd.exe","$d\Program Files (x86)\Steam\steamcmd.exe","$d\Users\$env:USERNAME\Downloads\steamcmd\steamcmd.exe","$d\Users\$env:USERNAME\Desktop\steamcmd\steamcmd.exe")) {
    ...
    foreach ($drive in $drives) {
        try {
            $found = Get-ChildItem -Path $drive -Recurse -Filter "steamcmd.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
...
if (Test-Path $CONFIG_FILE) {
    ...
    # Validate saved paths still exist — if not, re-detect
    $savedExe = "$SERVER_DIR\R5\Binaries\Win64\WindroseServer-Win64-Shipping.exe"
    if (-not (Test-Path $savedExe)) {
        Write-Host "Saved server path no longer valid - re-detecting..."
    Write-Host "Scanning drives for Windrose server installation..."
        $SERVER_DIR = $null
        $STEAMCMD   = $null
        Remove-Item $CONFIG_FILE -Force -ErrorAction SilentlyContinue
    }
} else {
    Write-Host "Scanning for Windrose server..."
    $SERVER_DIR = Find-ServerDir
    Write-Host "Scanning for SteamCMD..."
    $STEAMCMD   = Find-SteamCmd

    if (-not $SERVER_DIR) {
        Write-Host ""
        Write-Host "Could not auto-detect Windrose server." -ForegroundColor Yellow
        Write-Host "Enter the full path to your Windrose Dedicated Server folder."
        Write-Host "e.g. C:\SteamCMD\steamapps\common\Windrose Dedicated Server"
        Write-Host "e.g. D:\WindroseServer"
        Write-Host ""
        $SERVER_DIR = (Read-Host "Server folder path").Trim('"\`')
    }
    if (-not $STEAMCMD) {
        Write-Host ""
        Write-Host "Could not auto-detect SteamCMD." -ForegroundColor Yellow
        Write-Host "Enter full path to steamcmd.exe, or leave blank to skip auto-updates."
        Write-Host "e.g. C:\SteamCMD\steamcmd.exe"
        Write-Host ""
        $input = (Read-Host "SteamCMD path (blank to skip)").Trim('"\`')
        # If they gave a folder, append steamcmd.exe
        if ($input -and (Test-Path $input -PathType Container)) { $input = "$input\steamcmd.exe" }
        $STEAMCMD = $input
    }
...
$SETTINGS_FILE = "$SCRIPT_DIR\windrose_settings.json"
$script:webhookUrl   = ""
