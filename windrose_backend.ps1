...
function Find-ServerDir {
    ...
    foreach ($reg in $regPaths) {
        try {
            $steamPath = (Get-ItemProperty -Path $reg -ErrorAction SilentlyContinue).SteamPath
            if ($steamPath) {
                $steamPath = $steamPath -replace '/',''
                $candidates += "$steamPath\steamapps\common\Windrose Dedicated Server"
                # Read libraryfolders.vdf for extra Steam libraries
    ...
    # Fallback hardcoded paths covering all drives A-Z
    $drives = (Get-PSDrive -PSProvider FileSystem | Where-Object { $_.Root -match '^[A-Z]:\\$' }).Root
    foreach ($drive in $drives) {
        $d = $drive.TrimEnd('')
        $candidates += "$d\Steam\steamapps\common\Windrose Dedicated Server"
        $candidates += "$d\Program Files (x86)\Steam\steamapps\common\Windrose Dedicated Server"
        $candidates += "$d\Program Files\Steam\steamapps\common\Windrose Dedicated Server"
        $candidates += "$d\SteamLibrary\steamapps\common\Windrose Dedicated Server"
        $candidates += "$d\Games\Steam\steamapps\common\Windrose Dedicated Server"
        $candidates += "$d\Games\SteamLibrary\steamapps\common\Windrose Dedicated Server"
    }

    foreach ($c in ($candidates | Select-Object -Unique)) {
        try {
            $exe = "$c\R5\Binaries\Win64\WindroseServer-Win64-Shipping.exe"
            $job = Start-Job -ScriptBlock { param($p) Test-Path $p } -ArgumentList $exe
...
function Find-SteamCmd {
    ...
    foreach ($drive in $drives) {
        $d = $drive.TrimEnd('')
        $candidates += "$d\SteamCMD\steamcmd.exe"
        $candidates += "$d\steamcmd\steamcmd.exe"
        $candidates += "$d\steamcmd.exe"
        $candidates += "$d\Program Files (x86)\Steam\steamcmd.exe"
    }
    foreach ($c in ($candidates | Select-Object -Unique)) {
...
if (Test-Path $CONFIG_FILE) {
    ...
    if (-not $STEAMCMD) {
        ...
        Write-Host ""
        $STEAMCMD = (Read-Host "SteamCMD path (blank to skip)").Trim('"\`')
    }
